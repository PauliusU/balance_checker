...
def get_float_from_html_tag(html_string: str) -> float:
    ...
    if thousand_separated_numbers:
        ...
        if comma_separated_decimals:
            comma_number: str = re.sub(r"[^\d,]", "",
                                       thousand_separated_numbers[0])
            dot_number: str = comma_number.replace(",", ".")
            return float(dot_number)

        # If decimals are separated by dot:
        # Anything matching (that isn't a number or a decimal point
        # (dot)) is replaced with "", i.e. remains only numbers and decimal
        # points a.k.a remove comma and other symbols
        filtered_string: str = re.sub(r"[^\d\.]", "",
                                      thousand_separated_numbers[0])
        return float(filtered_string)

    decimal_only_separated_numbers: list = re.findall(r"\d+[.,]\d+",
                                                      html_string)
